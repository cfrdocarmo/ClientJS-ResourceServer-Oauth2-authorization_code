const config = {
  ...
  authorizeUrl: "http://auth.cfrdocarmofood.local:8081/oauth/authorize",
  tokenUrl: "http://auth.cfrdocarmofood.local:8081/oauth/token",
  callbackUrl: "http://www.foodanalytics.local:5500/foodanalytics/",
  cozinhasUrl: "http://www.cfrdocarmofood.local:8080/v1/cozinhas/"
};
...
function gerarAccessToken(code) {
  ...
  $.ajax({
    ...
    error: function(error) {
      alert("Erro ao gerar access token");
    }
  });
}
...
function login() {
  ...
  var headers = new Headers();
    ...
    headers.append('Authorization', 'Basic Zm9vZGFuYWx5dGljczpmb29kMTIz');
